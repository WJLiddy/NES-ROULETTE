...
Shoes.app do
	...
	@setup = stack :margin => 8 do
		...
		flow do
 		...
 		button "Roulette!" do
 			...
 			@game_list = RandomGameList.new("roms/snes/",@game_count_input.text.to_i,@play_published_input.checked?,@working_roms_input.checked?)			
			@game_started = false
			...
			#main logic loop
 			every(2) do
 				...
 				elsif Open3.capture2e('tasklist /fi "imagename eq zsnesw.exe"')[0].include?("INFO")
 					@game_started = false
	...
	def play
		...
		timer(1) do
			system 'ZSNES/zsnesw.exe "roms/snes/' + @game_list.current_game + '"'
		end
	end
end